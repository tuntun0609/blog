...
date: "2026-07-31"
category: "前端工程"
cover: "/blog/tanstack-query-quick-start.svg"
published: true
tags: ["React", "TanStack Query"]
---

我在项目里装 TanStack Query，通常是接口状态已经有点难管了。

比如从详情返回列表，又看到一次 loading。几个组件各自请求同一个接口。新增成功了，页面上的旧列表还没动。浏览器放了一会儿，回来看到的还是几分钟前的数据。这些问题叠到一起后，组件里很快就会堆出好几套 `useEffect + useState`。加载、错误、缓存和刷新规则散在各处，改起来挺费劲。

这篇用一个项目管理页往下做，场景有列表、筛选、详情、新增和分页。我平时也差不多按这个顺序接入。

TanStack Query 在这里负责记录数据何时请求、能复用多久、写操作完成后需要更新哪些缓存。
...
列表页来回切换时，我想复用上次结果。同一份数据可能被几个组件读取，筛选和页码也会改变请求。页面再加上新增、编辑以后，还得知道该刷新哪些列表。碰到这样的页面，我会直接上 TanStack Query。窗口重新聚焦、网络恢复后的后台更新也能一起处理。

只在 Server Component 里读取一次的数据，我一般直接 `fetch`。它没有客户端缓存的生命周期，代码留在服务端更省事。

写代码时，我把 `queryKey` 当作缓存地址。取数据的函数放在 `queryFn` 里。
...
列表稳定以后，再把新增或编辑操作接到 `useMutation`，成功回调里让相关缓存失效。详情有前置 ID 就加 `enabled`，分页出现闪空再补 `placeholderData: keepPreviousData`。
