...
“我这里明明能跑”这句话，多半是在说：程序依赖的环境没有一起交出去。系统库、运行时版本、配置和启动命令，只要有一个不一样，换台电脑就可能出问题。

Docker 会把应用和它需要的环境打包成**镜像**，再用镜像启动相互隔离的**容器**。这样交付的就不只是一份“你照着配一下”的说明，而是一套能反复跑起来的环境。

Docker 的功能很多，刚开始不用全学。下面按最常用的顺序来：先跑一个现成服务，再看容器和数据怎么管，最后自己构建镜像，并用 Docker Compose 一次启动多个服务。

## 先建立一张概念地图

刚接触 Docker 时，最容易混的是镜像和容器。先把下面几件事记住就够了：
...
一个镜像可以启动很多个容器。删容器不会删镜像；更重要的是，删容器也不该把数据库数据一起删掉，所以要长期保留的数据得放进卷里。
